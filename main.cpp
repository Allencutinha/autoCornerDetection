...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        const char *s = argv[i];
        if (strcmp(s, "-w") == 0) {
            if (sscanf(argv[++i], "%u", &board_size.width) != 1 ||
                board_size.width <= 0) {
            ...
            if (sscanf(argv[++i], "%u", &board_size.height) != 1 ||
                board_size.height <= 0) {
    ...
    if (input_filename) {
        ...
        // capture = cvCreateFileCapture( input_filename ); //OBRAND commented
        // out
        if (!capture) {
    ...
    // Allocate memory
    elem_size =
        board_size.width * board_size.height * sizeof(image_points_buf[0]);
    ...
    for (int j = 1;; j++) {
        ...
        if (f && fgets(imagename, sizeof(imagename) - 2, f)) {
            ...
            std::string imagename_str = "../VMRImage0.jpg";
            // imagename = imagename_str.c_str();
        ...
        // ...Or capture the correct frame from the video
            ...
            if (view0) {
                view = cvCreateImage(cvGetSize(view0), IPL_DEPTH_8U,
                                     view0->nChannels);
        ...
        if (method == 0) {
            ...
            found = cvFindChessboardCorners3(view, board_size, image_points_buf,
                                             &count, min_number_of_corners);

            found = findChessboardCorners3(img, board_size, image_points_buf,
                                           &count, min_number_of_corners);
        }
