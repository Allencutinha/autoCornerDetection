...
cameras by Martin Rufli during his Master Thesis under supervision of Davide
...
static int mrAugmentBestRun(CvCBQuad *new_quads, int new_quad_count,
                            int new_dilation, CvCBQuad **old_quads,
                            int old_quad_count, int old_dilation);
...
static void mrLabelQuadGroup(CvCBQuad **quad_group, int count,
                             CvSize pattern_size, bool firstRun);

static void mrCopyQuadGroup(CvCBQuad **temp_quad_group,
                            CvCBQuad **out_quad_group, int count);

static int icvCleanFoundConnectedQuads(int quad_count, CvCBQuad **quads,
                                       CvSize pattern_size);

static int mrWriteCorners(CvCBQuad **output_quads, int count,
                          CvSize pattern_size, int min_number_of_corners);
...
// START TIMER
    ...
    {
    error << "In this implementation only square sized checker boards are
    supported" << endl; error.close(); return -1;
    }
    */
    if (!out_corners) {
...
// EVALUATE TIMER
    ...
    FindChessboardCorners2.open("timer/FindChessboardCorners2.txt",
                                ofstream::app);
    FindChessboardCorners2 << "Time 0.1 for cvFindChessboardCorners2 was "
                           << time0_1 << " seconds." << endl;
#endif
    ...
    cvAdaptiveThreshold(img, thresh_img, 255, CV_ADAPTIVE_THRESH_GAUSSIAN_C,
...
// EVALUATE TIMER
        ...
        FindChessboardCorners2 << "Time 0.2 for cvFindChessboardCorners2 was "
                               << time0_2 << " seconds." << endl;
#endif
...
// EVALUATE TIMER
        ...
        FindChessboardCorners2 << "Time 0.3 for cvFindChessboardCorners2 was "
                               << time0_3 << " seconds." << endl;
#endif
...
// VISUALIZATION--------------------------------------------------------------
        ...
        CV_CALL(quad_count =
                    icvGenerateQuads(&quads, &corners, storage, thresh_img,
                                     flags, dilations, true));
        if (quad_count <= 0)
            continue;
...
// EVALUATE TIMER
        ...
        FindChessboardCorners2 << "Time 0.4 for cvFindChessboardCorners2 was "
                               << time0_4 << " seconds." << endl;
#endif
...
// VISUALIZATION--------------------------------------------------------------
        ...
        for (group_idx = 0;; group_idx++) {
            int count;
            CV_CALL(count =
                        icvFindConnectedQuads(quads, quad_count, quad_group,
                                              group_idx, storage, dilations));
...
// VISUALIZATION--------------------------------------------------------------
                ...
                IplImage *imageCopy11 =
                    cvCreateImage(cvGetSize(thresh_img), 8, 3);
                        ...
                        for (int k = 0; k < count; k++) {
                            for (int l = 0; l < 4; l++) {
                                if (((quad_group[k])->corners[l]->row == i) &&
                                    ...
                                    // color than border corners
                                    if ((quad_group[k])
                                            ->corners[l]
                                            ->needsNeighbor == false) {
                                        cv::putText(imageCopy11Mat, str, ptt,
                                    ...
                                    cvShowImage("Corners in increasing order",
                                                imageCopy11);
                                    // cvSaveImage("pictureVis/CornersIncreasingOrder.tif",
                                    // imageCopy11); cvWaitKey(0);
                                }
                            }
                        }
                    }
                ...
                // Allocate memory
                CV_CALL(output_quad_group =
                            (CvCBQuad **)cvAlloc(sizeof(output_quad_group[0]) *
                                                 ((pattern_size.height + 2) *
...
// EVALUATE TIMER
    ...
    FindChessboardCorners2.open("timer/FindChessboardCorners2.txt",
                                ofstream::app);
    FindChessboardCorners2 << "Time 1 for cvFindChessboardCorners2 was "
                           << time1 << " seconds." << endl;
#endif
...
// VISUALIZATION--------------------------------------------------------------
        ...
        for (int kkk = 0; kkk < max_count; kkk++) {
            ...
            cv::fillConvexPoly(imageCopy23Mat, pt, 4,
...
// VISUALIZATION--------------------------------------------------------------
            ...
            if (feedBack == -1) {
                ...
                for (int kkk = max_count; kkk < max_count + 1; kkk++) {
                    ...
                    pt[3].x = (int)print_quad->corners[3]->pt.x;
                    pt[3].y = (int)print_quad->corners[3]->pt.y;
                    cv::line(imageCopy23Mat, pt[0], pt[1], CV_RGB(255, 0, 0), 2,
                             8);
                    cv::line(imageCopy23Mat, pt[1], pt[2], CV_RGB(255, 0, 0), 2,
                             8);
                    cv::line(imageCopy23Mat, pt[2], pt[3], CV_RGB(255, 0, 0), 2,
                             8);
                    cv::line(imageCopy23Mat, pt[3], pt[0], CV_RGB(255, 0, 0), 2,
                             8);
                }
                ...
                for (int kkk = 0; kkk < max_count; kkk++) {
                    ...
                    for (int kkkk = 0; kkkk < 4; kkkk++) {
                        if (print_quad->neighbors[kkkk] == remember_quad) {
                            ...
                            pt[3].x = (int)print_quad->corners[3]->pt.x;
                            pt[3].y = (int)print_quad->corners[3]->pt.y;
                            cv::line(imageCopy23Mat, pt[0], pt[1],
                                     CV_RGB(255, 0, 0), 2, 8);
                            cv::line(imageCopy23Mat, pt[1], pt[2],
                                     CV_RGB(255, 0, 0), 2, 8);
                            cv::line(imageCopy23Mat, pt[2], pt[3],
                                     CV_RGB(255, 0, 0), 2, 8);
                            cv::line(imageCopy23Mat, pt[3], pt[0],
                                     CV_RGB(255, 0, 0), 2, 8);
                        }
                    }
                }
                cvShowImage("PART2: Starting Point", imageCopy23);
                cvSaveImage("pictureVis/part2StartAndSelectedQuad.png",
                            imageCopy23);
                cvWaitKey(0);
            }
#endif
            ...
            if (feedBack == -1) {
                ...
                mrLabelQuadGroup(output_quad_group, max_count, pattern_size,
                                 false);
    ...
    */
    ...
    if (found == -1) {
        error << "While linking the corners a problem was encountered. No "
...
// EVALUATE TIMER
    ...
    FindChessboardCorners2 << "Time 3 for cvFindChessboardCorners2 was "
                           << time3 << " seconds." << endl;
    ...
    // -1  ->	Error or corner linking problem, see error.txt for more
    // information
...
// EVALUATE TIMER
    ...
    FindConnectedQuads << "Time for cvFindConnectedQuads was " << time
                       << " seconds." << endl;
...
// START TIMER
    ...
    for (int i = min_row; i <= max_row; i++) {
        for (int j = min_column; j <= max_column; j++) {
            ...
            // 1	is a border corner or a conrer which still needs a
            // neighbor 2	is a fully connected internal corner >2
            // something went wrong during labeling, report a warning
            int number = 1;
...
// EVALUATE TIMER
    ...
    LabelQuadGroup << "Time for mrLabelQuadGroup was " << time << " seconds."
                   << endl;
...
static void mrCopyQuadGroup(CvCBQuad **temp_quad_group,
                            CvCBQuad **for_out_quad_group, int count) {
    for (int i = 0; i < count; i++) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            for_out_quad_group[i]->corners[j]->pt.x =
                temp_quad_group[i]->corners[j]->pt.x;
            for_out_quad_group[i]->corners[j]->pt.y =
                temp_quad_group[i]->corners[j]->pt.y;
            for_out_quad_group[i]->corners[j]->row =
                temp_quad_group[i]->corners[j]->row;
            for_out_quad_group[i]->corners[j]->column =
                temp_quad_group[i]->corners[j]->column;
            for_out_quad_group[i]->corners[j]->needsNeighbor =
                temp_quad_group[i]->corners[j]->needsNeighbor;
        }
    }
}
...
static void mrFindQuadNeighbors2(CvCBQuad *quads, int quad_count,
                                 int dilation) {
// START TIMER
#if TIMER
            ...
            for (k = 0; k < quad_count; k++) {
                ...
                for (j = 0; j < 4; j++) {
                    ...
                    if (dist < min_dist &&
                        ...
                        // or column don't link. Check from the current corner's
                        // view, whether the corner diagonal from the candidate
                        // corner is also on the same side of the two lines as
                        // the current corner and the candidate corner.
                        ...
                        // candidate quad compute midpoints of "parallel" quad
                        // sides 1
                        ...
                        // Then make shure that two border quads of the same row
                        // or column don't link. Check from the candidate
                        // corner's view, whether the corner diagonal from the
                        // current corner is also on the same side of the two
                        // lines as the current corner and the candidate corner.
...
// EVALUATE TIMER
    ...
    FindQuadNeighbors2 << "Time for mrFindQuadNeighbors2 was " << time
                       << " seconds." << endl;
    ...
    for (idx = 0; idx < old_quad_count; idx++) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            for (k = 0; k < new_quad_count; k++) {
                ...
                for (j = 0; j < 4; j++) {
                    ...
                    if ((dist < min_dist) &&
                        ...
                        // than corner "i". This is given, if the cross product
                        // has a different sign for both computations below:
                        float a5 = x6 - x5;
                        float b5 = y6 - y5;
                        // the current corner
            ...
            if (closest_corner_idx >= 0 && min_dist < FLT_MAX) {
                ...
                old_quads[old_quad_count]->group_idx =
                    cur_quad->group_idx; // the same as the current quad
                old_quads[old_quad_count]->labeled =
                    false; // do it right afterwards

                // We only know one neighbor for shure, initialize rest with
                // the NULL pointer
                old_quads[old_quad_count]->neighbors[closest_corner_idx] =
                    cur_quad;
                old_quads[old_quad_count]
                    ->neighbors[(closest_corner_idx + 1) % 4] = NULL;
                old_quads[old_quad_count]
                    ->neighbors[(closest_corner_idx + 2) % 4] = NULL;
                old_quads[old_quad_count]
                    ->neighbors[(closest_corner_idx + 3) % 4] = NULL;

                for (int j = 0; j < 4; j++) {
                    old_quads[old_quad_count]->corners[j] = new CvCBCorner;
                    old_quads[old_quad_count]->corners[j]->pt.x =
                        closest_quad->corners[j]->pt.x;
                    old_quads[old_quad_count]->corners[j]->pt.y =
                        closest_quad->corners[j]->pt.y;
                }
...
// EVALUATE TIMER
    ...
    AugmentBestRun << "Time for mrAugmentBestRun was " << time << " seconds."
                   << endl;
...
// START TIMER
    ...
    CV_CALL(root =
                cvCreateSeq(0, sizeof(CvSeq), sizeof(CvSeq *), temp_storage));
    ...
    while ((src_contour = cvFindNextContour(scanner)) != 0) {
        ...
        // squares are detected. During PART 2, we allow the polygonial
        // approcimation function below to approximate more freely, which can
        ...
        if (CV_IS_SEQ_HOLE(src_contour) &&
            ...
            if (dst_contour->total == 4 &&
                ...
                if (true) //!(flags & CV_CALIB_CB_FILTER_QUADS) ||
                          // d3*4 > d4 && d4*4 > d3 && d3*d4 < area*1.5 && area
                          // > min_size && d1 >= 0.15 * p && d2 >= 0.15 * p )
                {
...
// EVALUATE TIMER
    ...
    GenerateQuads << "Time for icvGenerateQuads was " << time << " seconds."
                  << endl;
    GenerateQuads.close();
#endif
...
static int mrWriteCorners(CvCBQuad **output_quads, int count,
    ...
    // exactly how the checkerboard is oriented. Else we need to prepare enought
    ...
    for (int i = min_row + 1; i < maxPattern_sizeRow + min_row + 1; i++) {
        for (int j = min_column + 1; j < maxPattern_sizeColumn + min_column + 1;
            ...
            for (int k = 0; k < count; k++) {
                for (int l = 0; l < 4; l++) {
                    if (((output_quads[k])->corners[l]->row == i) &&
                        ((output_quads[k])->corners[l]->column == j)) {
                        // Only write corners to the output file, which are
                        // connected i.e. only if iter == 2
                        if (iter == 2) {
                            // The respective row and column have been found,
                            // print it to the output file, only do this once
